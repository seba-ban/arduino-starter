...
### About the flyback diode

The 1N4007 diode goes directly across the two motor wires — but **direction matters!**

```
   Motor wire 2 (→ 5V)
         |
       ──┤ (silver stripe / cathode end)
       ──┤
         | (no stripe / anode end)
   Motor wire 1 (→ Collector)
```

- **Stripe end (cathode)** → the motor wire that goes to **5V**
- **No-stripe end (anode)** → the motor wire that goes to the **Collector**

When you switch the motor off, it tries to keep current flowing (motors are like tiny magnets — they resist change).
This creates a sudden reverse voltage spike.
Without the diode, that spike can damage the transistor or the Arduino.
The diode gives that spike a safe path to flow away harmlessly.

### About the capacitor

The 100nF ceramic capacitor also goes directly across the two motor wires.
It absorbs high-frequency electrical noise when the motor changes speed.
This is a *different* kind of protection from the diode — you need both!
...
6. Connect the **1N4007 diode** across the two motor wires — **stripe end toward 5V**, no-stripe end toward the Collector.
7. Connect the **100nF capacitor** across the two motor wires (one leg on each — any direction).
8. Connect the potentiometer: left leg → **GND**, middle leg → **A0**, right leg → **5V**.
...
| 1N4007 diode (stripe end) | Motor wire 2 (the 5V side) |
| 1N4007 diode (no-stripe end) | Motor wire 1 (the Collector side) |
